...
export default class DateRangePicker {
  constructor(element, options = {}, cb = () => {}) {
    ...
    // Independence year and month click
    this.yearMonthSingle = false;
    this.ranges = {};
    ...
    if (typeof options.yearMonthSingle === 'boolean') this.yearMonthSingle = options.yearMonthSingle;
  ...
  renderCalendar(side) {
    ...
    const calendar1 = side === 'left' ? this.leftCalendar : this.rightCalendar;
    const month = calendar1.month.month();
    const year = calendar1.month.year();
    const hour = calendar1.month.hour();
    const minute = calendar1.month.minute();
    const second = calendar1.month.second();
    ...
    if (this.showDropdowns) {
      ...
      for (let m = 0; m < 12; m++) {
        if ((!inMinYear || (minDate && m >= minDate.month())) && (!inMaxYear || (maxDate && m <= maxDate.month()))) {
          monthHtml += `<option value="${m}" ${m === currentMonth ? 'selected="selected"' : ''}>${
            this.locale.monthNames[m]
          }</option>`;
        } else {
          monthHtml += `<option value="${m}" ${m === currentMonth ? 'selected="selected"' : ''} disabled='disabled'>${
            this.locale.monthNames[m]
          }</option>`;
        }
      }
    ...
    if (
      ...
      if (this.yearMonthSingle) {
        html += `<th colspan="1" class="month">${monthHtml}</th>`;
        html += '<th class="next available"><span></span></th>';

        if ((!minDate || minDate.isBefore(calendar.firstDay, 'year')) && (!this.linkedCalendars || side === 'left')) {
          html += '<th class="prevYear available"><span></span></th>';
        } else {
          html += '<th></th>';
        }

        html += `<th colspan="2" class="year">${yearHtml}</th>`;
        html += '<th class="nextYear available"><span></span></th>';
      } else {
        html += '<th class="next available"><span></span></th>';
      }
    } else {
    ...
    $.each(this.locale.daysOfWeek, function (index, dayOfWeek1) {
    ...
    for (let x = 0; x < 6; x++) {
      ...
      // add week number
      if (this.showWeekNumbers) html += `<td class="week">${calendar[x][0].week()}</td>`;
      else if (this.showISOWeekNumbers) html += `<td class="week">${calendar[x][0].isoWeek()}</td>`;

      for (let y = 0; y < 7; y++) {
        const classes = [];

        // highlight today's date
        if (calendar[x][y].isSame(new Date(), 'day')) classes.push('today');

        // highlight weekends
        if (calendar[x][y].isoWeekday() > 5) classes.push('weekend');

        // grey out the dates in other months displayed at beginning and end of this calendar
        if (calendar[x][y].month() !== calendar[1][1].month()) classes.push('off', 'ends');

        // don't allow selection of dates before the minimum date
        if (this.minDate && calendar[x][y].isBefore(this.minDate, 'day')) classes.push('off', 'disabled');

        // don't allow selection of dates after the maximum date
        if (maxDate && calendar[x][y].isAfter(maxDate, 'day')) classes.push('off', 'disabled');

        // don't allow selection of date if a custom function decides it's invalid
        if (this.isInvalidDate(calendar[x][y])) classes.push('off', 'disabled');

        // highlight the currently selected start date
        if (calendar[x][y].format('YYYY-MM-DD') === this.startDate.format('YYYY-MM-DD'))
          classes.push('active', 'start-date');

        // highlight the currently selected end date
        if (this.endDate != null && calendar[x][y].format('YYYY-MM-DD') === this.endDate.format('YYYY-MM-DD'))
          classes.push('active', 'end-date');

        // highlight dates in-between the selected dates
        if (this.endDate != null && calendar[x][y] > this.startDate && calendar[x][y] < this.endDate)
          classes.push('in-range');

        // apply custom classes for this date
        const isCustom = this.isCustomDate(calendar[x][y]);
        if (isCustom !== false) {
        ...
        html += `<td class="${cname.replace(/^\s+|\s+$/g, '')}" data-title="r${x}c${y}">${calendar[x][y].date()}</td>`;
      }
      html += '</tr>';
    }
  ...
  renderTimePicker(side) {
    ...
    if (side === 'left') {
      ...
      if (timeSelector.html() !== '') {
        selected.hour(
          !Number.isNaN(selected.hour()) ? selected.hour() : timeSelector.find('.hourselect option:selected').val(),
        );
        selected.minute(
          !Number.isNaN(selected.minute())
            ? selected.minute()
            : timeSelector.find('.minuteselect option:selected').val(),
        );
        selected.second(
          !Number.isNaN(selected.second())
            ? selected.second()
  ...
  show(e) {
    ...
    this._outsideClickProxy = $.proxy(function (ev) {
  ...
  clickPrevYear(e) {
    ...
    if (cal.hasClass('left')) {
      ...
      if (this.linkedCalendars) this.rightCalendar.month.subtract(12, 'month');
    } else {
    ...
    let year = moment(cal.find('.yearselect').val(), 'YYYY').subtract(1, 'year').year();
  ...
  clickNextYear(e) {
    ...
    if (cal.hasClass('left')) {
      ...
      if (this.linkedCalendars) this.leftCalendar.month.add(12, 'month');
    }
