...
export default class DateRangePicker {
  constructor(element, options = {}, cb = () => {}) {
    ...
    if (typeof options.customHeader === 'boolean') this.customHeader = options.customHeader;
  ...
  setMinDate(minDate) {
    ...
    if (this.minDate && this.startDate.isBefore(this.minDate)) {
      ...
      // this.setStartDate(this.minDate.clone());
      if (this.timePicker && this.timePickerIncrement)
        this.minDate.minute(
          Math.round(this.minDate.minute() / this.timePickerIncrement) * this.timePickerIncrement,
        );
    }
    if (this.maxDate && this.minDate.isAfter(this.maxDate)) {
      this.minDate = this.maxDate.clone();
      if (this.timePicker && this.timePickerIncrement)
        this.minDate.minute(
          Math.floor(this.minDate.minute() / this.timePickerIncrement) * this.timePickerIncrement,
        );
    }
  ...
  renderCalendar(side) {
    ...
    if (this.customHeader) {
      ...
      if ((!minDate || minDate.isBefore(calendar.firstDay)) && (!this.linkedCalendars || side == 'left')) {
        html += '<div class="prev available"><div><span></span></div></div>';
      } else {
        html += '<div></div>';
      }
      ...
      if (this.showDropdowns) {
        ...
        for (var m = 0; m < 12; m++) {
          if ((!inMinYear || (minDate && m >= minDate.month())) && (!inMaxYear || (maxDate && m <= maxDate.month()))) {
            ...
            if (!inMaxYear) {
              monthHtml += " disabled='disabled'>" + this.locale.monthNames[m] + '</option>';
            } else {
              monthHtml += '>' + this.locale.monthNames[m] + '</option>';
            }
          }
        }
        monthHtml += '</select>';
      ...
      if (
        ...
        if (this.yearMonthSingle) {
          html += '<div class="month">' + monthHtml + '</div>';

          if (inMaxYear && currentMonth >= 11) {
            html += '<div></div>';
          } else {
            html += '<div class="next available"><div><span></span></div></div>';
          }

          if ((!minDate || minDate.isBefore(calendar.firstDay, 'year')) && (!this.linkedCalendars || side == 'left')) {
            html += '<div class="prevYear available"><div><span></span></div></div>';
          } else {
            html += '<div></div>';
          }

          html += '<div class="year">' + yearHtml + '</div>';

          if (!inMaxYear) {
            html += '<div class="nextYear available"><div><span></span></div></div>';
          } else {
            html += '<div></div>';
          }
        } else {
  ...
  outsideClick(e) {
    ...
    if (
      // ie modal dialog fix
      (e.type == 'focusin' && $(e.target).is($(this.element))) ||
  ...
  clickPrevYear(e) {
    ...
    var year = moment(cal.find('.yearselect').val(), 'YYYY').subtract(1, 'year').year();
